 ...
 * fabric photo; this module runs the prediction: a transparent, deterministic
 * rule model that combines dye knowledge (temp, duration, mordant, baseline
 * fastness), fabric uptake factors and a wash-fade curve, then computes
 * CIEDE2000 colour difference between the original and the simulated
 * after-wash colour.
 *
...
import { DYE_KNOWLEDGE, WASH_CYCLES } from "./constants";
import {
  deltaE2000,
  fadeTowardPaper,
  hexToRgb,
...
function hashToNumber(input: string | undefined): number {
  let n = 0;
...
function nameHue(hex: string): string {
...
function retentionCategory(retention: number): string {
...
// ---------------------------------------------------------------------------
// The prediction model (pure function — reused by mutation, seed and HTTP)
// ---------------------------------------------------------------------------
